...
- **요약 결과 표시**: 요약 자체(한 줄 요약 + 포인트)는 스트리밍 없이 완료된 형태로 한 번에 표시한다. 다만 어떤 무료 모델을 시도, 실패했는지에 대한 진행 상황은 SSE로 실시간 스트리밍해 화면에 보여준다(배포 후 추가, `/api/summarize`, `/api/resummarize`).
- 자세한 아키텍처, 컴포넌트, 에러 처리, 테스트 범위는 설계 문서 참고. 단, 설계 문서(2026-07-28)는 최초 버전 기준이며, 배포 후 다음 기능이 추가로 반영됨: PDF 첫 페이지 클라이언트 썸네일, 추출 텍스트 화면 표시(`/api/extract` 분리), "반복 요약하기"(직전 요약을 더 압축하는 별도 프롬프트, `/api/resummarize`), 요약 실패 시 파일 재선택 없이 재시도.
